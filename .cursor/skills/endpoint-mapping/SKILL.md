...
1. `openapi.json` — necessary, **not sufficient**. `OpenAPI::SCOPE_IGNORE` hides live OCS (filenames sanitization, appstore discover). 301/301 OpenAPI coverage does not mean the map is machine-complete.
2. `core/routes.php`, `apps/*/appinfo/routes.php`
3. Controller attributes: `ApiRoute`, `FrontpageRoute`, `Route` — attribute routes missing from `routes.php` are still public contracts
4. DAV plugins / Sabre registrations (including principal prefixes, not only `RootCollection` children)
5. `ocs-provider/`, `.well-known`, `status.php`, `index.php` entrypoints
6. `remote.php`, `public.php`, `cron.php` only as protocol gates — map the APIs they front, not the PHP files as endpoints

Do not add `/.well-known/webfinger` or `/.well-known/nodeinfo` unless this repo registers a handler (catch-all `WellKnownController#handle` 404s are not product rows).
