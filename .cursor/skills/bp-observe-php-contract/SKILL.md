...
| Files templates OCS | fields `data: object`; create 403 `data: any` | `list`/`listTemplateFields` → JSON **arrays**; unknown `fileId` → 200 `[]`; create 403 `data:[]` + meta.message (`File already exists` only distinct); `path` at `…/templates/path` not directEditing; init failures usually 200 `template_path:''` |
| ShareInfo `#info` | `auth: session`; bare ShareInfo 200; 401 | `PublicPage`; `{data,status}` wrap on all JSON; outgoing S2S off → 404 wrapped; missing `t` → raw 400; body password not public-link session |
| `dav.Public#tree` / `#legacy-webdav` | success 207 multistatus | per-method 200/201/207; `auth: public-share` correct; owner cookie does not auth; v2 non-GET and legacy all methods need AJAX or outgoing S2S; password session `public_link_authenticated` (share id list) |
