...
| DAV calendars store | `/api/parity/reset-dav-calendars-store` | `resetParityCalendarsStores()` in `dav-calendars.parity.test.ts`, `dav-legacy-caldav.parity.test.ts`, and `dav-cal-contacts-io.parity.test.ts`; seed via `/api/parity/seed-dav-calendars` |
...
| Provisioning user accounts | `/api/parity/reset-provisioning-store` | `resetParityProvisioningStores()` in `provisioning-self-read.parity.test.ts` and `provisioning-phone-search.parity.test.ts` |
| Provisioning default phone region | `/api/parity/set-provisioning-config` | `setParityDefaultPhoneRegion(region)` in `provisioning-phone-search.parity.test.ts` |
...
- A legacy mock 404 of the form `Legacy mock has no fixture for <METHOD> <path>` on **every** case of one HTTP method, while other methods on the same path pass, is the adapter's method gate, not the domain handler. `fetchLegacyMockSnapshot` has an early allowlist (`GET`/`POST`/`PUT`/`PATCH`/`DELETE`/`PROPFIND`/`OPTIONS`) that returns that 404 before most domain handlers run. Registering the path in `hasLegacyMockFixture` does not help — its own method gate sits *after* the per-domain path checks, so a method can be admitted there and still be rejected upstream. When adding a method the mock has never served, add it to the `fetchLegacyMockSnapshot` allowlist (or route the handler above the gate), and rerun the whole parity suite: the allowlist is global, so a newly admitted method now falls through to every handler below it.
