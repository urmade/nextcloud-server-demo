...
**Trap:** Unified sharing v1 `updateShareState` / `updateShareUserStatus` invalid enum → HTTP **400** with `ocs.data` as PHP `ValueError` message (e.g. `"nope" is not a valid backing value for enum NCU\\Sharing\\ShareState`). `updateShareState` does not catch `ShareInvalidException`; `selectSharePermissionPreset` map 400 is a lie.
